...
public class JavaLanguageJudgeStrategy implements JudgeStrategy {
    ...
    public JudgeInfo doJudge(JudgeContext judgeContext) {
        ...
        if (outputList == null || inputList == null || judgeCaseList == null
                || outputList.size() != inputList.size()
                || judgeCaseList.size() != inputList.size()) {
        ...
        Long needMemoryLimit = judgeConfig == null ? null : judgeConfig.getMemoryLimit();
        Long needTimeLimit = judgeConfig == null ? null : judgeConfig.getTimeLimit();
        if (needMemoryLimit != null && memory > needMemoryLimit) {
            judgeInfoMessageEnum = JudgeInfoMessageEnum.MEMORY_LIMIT_EXCEEDED;
            judgeInfoResponse.setMessage(judgeInfoMessageEnum.getValue());
            return judgeInfoResponse;
        }
        if (needTimeLimit != null && time > needTimeLimit) {
