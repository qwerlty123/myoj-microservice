...
alter table question_submit
    add column if not exists retryCount int default 0 not null comment '判题执行重试次数' after userId,
    add column if not exists lastError varchar(1024) null comment '最近一次错误' after retryCount,
    add column if not exists nextRetryTime datetime null comment '下一次判题执行调度时间' after lastError;
